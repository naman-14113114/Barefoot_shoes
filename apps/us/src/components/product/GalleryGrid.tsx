...
function getAltText(src: string, title: string, index: number): string {
  try {
    ...
    if (filename.startsWith("buudy-barefoot-shoes-") || filename.startsWith("juujo-barefoot-shoes-")) {
      const clean = filename.replace(/^(?:buudy|juujo)-barefoot-shoes-/, "");
      const ltMatch = clean.match(/^(?:lt-03-premium-nappa-(?:white|black|green|dark-blue|light-blue|pink)|lt-02-rugged-(?:white|white-black|beige-red|all-black|black)|lt-01-court-lite-(?:gum-white|white|nubuck-all-black|nubuck-ash-grey|nubuck-essence-white|nubuck-suede-sand|suede-sand|essence-white|all-black|ash-grey|black)|lt-01-essence-(?:black|jet-black)|rs-01-runner-(?:desert-taupe|taupe|all-black|jet-black|anthracite|forest-green)|lt-01-platform-(?:white|all-white))-(.*)$/);
      if (ltMatch) {
